/**
 * IOL Broadcasting — Cloudflare Worker v6
 * Endpoints:
 *   GET /all|news|sport|...      → IOL RSS feeds as JSON
 *   GET /shorten?url=            → TinyURL shortener
 *   GET /image?url=              → CORS image proxy for iol-prod.appspot.com
 */
...
export default {
  async fetch(request) {
    ...
    // ── Image proxy — fetches IOL images server-side, adds CORS headers ──
    if (section === 'image') {
      ...
      try {
        ...
        const imgBody     = await res.arrayBuffer();
    ...
    if (section === 'shorten') {
      ...
      try {
        ...
        if (!s.startsWith('http')) throw new Error('TinyURL returned invalid response');
