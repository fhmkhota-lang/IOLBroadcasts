...
/* ── Category colours ── */
const CAT_COL = {
  news:'#E8192C', politics:'#E8192C', sport:'#00A651',
  business:'#1565C0', technology:'#1565C0', entertainment:'#E8192C',
  motoring:'#F26522', lifestyle:'#E91E8C', travel:'#F5A623', default:'#E8192C'
};
const CAT_LABEL = {
  news:'NEWS', politics:'NEWS', sport:'SPORT', business:'BUSINESS',
  technology:'TECH', entertainment:'ENTERTAINMENT', motoring:'MOTORING',
  lifestyle:'LIFESTYLE', travel:'TRAVEL', default:'NEWS'
};
function col(c)   { return CAT_COL[(c||'').toLowerCase()]   || CAT_COL.default; }
function label(c) { return CAT_LABEL[(c||'').toLowerCase()] || CAT_LABEL.default; }
...
let designer = {
  ...
  headline: 'Your story headline goes here',
  caption: 'Add a caption or key detail about this story.',
  imageUrl: '',
  imageEl: null,          // loaded Image object
  imgOffsetX: 0,          // pan offset
  imgOffsetY: 0,
  imgScale: 1,
  slides: [],             // carousel slides [{kicker,headline,caption,imageUrl,imageEl,imgOffsetX,imgOffsetY,imgScale}]
  currentSlide: 0,
  isDragging: false,
  dragStartX: 0,
  dragStartY: 0,
...
/* Open card tab → populate selector */
document.querySelectorAll('.nav-tab').forEach(tab => {
  tab.addEventListener('click', () => {
    if (tab.dataset.tab === 'cards') {
      populateCardStorySelector();
...
const storySelEl = document.getElementById('card-story-select');
if (storySelEl) {
  storySelEl.addEventListener('change', async function() {
    ...
    if (!s) return;
    // Pre-fill designer state from story
    designer.headline  = s.headline || '';
    designer.caption   = s.excerpt  || '';
    ...
    if (designer.imageUrl) {
      designer.imageEl = await tryLoadImg(designer.imageUrl);
    } else {
      designer.imageEl = null;
    }
    renderCard();
  });
}

function autoKicker(headline, cat) {
  if (!headline) return (CAT_LABEL[(cat||'').toLowerCase()] || 'BREAKING NEWS');
  return headline.split(' ').slice(0,3).join(' ').toUpperCase();
}

/* Sync UI input elements from designer state */
function syncUIFromState() {
  const fields = {
    'cd-kicker':   designer.kicker,
    'cd-headline': designer.headline,
    'cd-caption':  designer.caption,
    'cd-imgurl':   designer.imageUrl,
    'cd-category': designer.category,
  };
...
document.getElementById('card-type-pills')?.addEventListener('click', e => {
  ...
  if (designer.type === 'carousel' && designer.slides.length === 0) {
    // Seed with current state as first slide
    addCarouselSlide();
  }
  updateCarouselUI();
  renderCard();
...
['cd-kicker','cd-headline','cd-caption','cd-category'].forEach(id => {
  document.getElementById(id)?.addEventListener('input', function() {
    ...
    if (designer.type === 'carousel' && designer.slides.length > 0) {
      ...
      if (id === 'cd-kicker')   sl.kicker   = this.value;
      if (id === 'cd-headline') sl.headline = this.value;
      if (id === 'cd-caption')  sl.caption  = this.value;
      if (id === 'cd-category') sl.category = this.value;
    }
    renderCard();
  });
});
...
document.getElementById('cd-imgurl')?.addEventListener('change', async function() {
  designer.imageUrl = this.value.trim();
  designer.imgOffsetX = 0; designer.imgOffsetY = 0; designer.imgScale = 1;
  if (designer.imageUrl) {
    designer.imageEl = await tryLoadImg(designer.imageUrl);
  } else {
  ...
  if (designer.type === 'carousel' && designer.slides.length > 0) {
    const sl = designer.slides[designer.currentSlide];
    sl.imageUrl = designer.imageUrl; sl.imageEl = designer.imageEl;
    sl.imgOffsetX = 0; sl.imgOffsetY = 0; sl.imgScale = 1;
  }
  renderCard();
});
...
if (cardCanvas) {
  cardCanvas.addEventListener('mousedown', e => {
    ...
    designer.dragStartX = e.clientX;
    designer.dragStartY = e.clientY;
    const sl = currentSlideData();
    designer.dragStartOX = sl.imgOffsetX || 0;
    designer.dragStartOY = sl.imgOffsetY || 0;
    e.preventDefault();
  });
  window.addEventListener('mousemove', e => {
    if (!designer.isDragging) return;
    const rect   = cardCanvas.getBoundingClientRect();
    const scaleX = SIZE / rect.width;
    const dx     = (e.clientX - designer.dragStartX) * scaleX;
    const dy     = (e.clientY - designer.dragStartY) * scaleX;
    const sl     = currentSlideData();
    sl.imgOffsetX = designer.dragStartOX + dx;
    sl.imgOffsetY = designer.dragStartOY + dy;
    if (designer.type === 'single') {
      designer.imgOffsetX = sl.imgOffsetX;
      designer.imgOffsetY = sl.imgOffsetY;
    }
    renderCard();
  });
  ...
  cardCanvas.addEventListener('touchstart', e => {
    ...
    designer.isDragging = true;
    designer.dragStartX = e.touches[0].clientX;
    designer.dragStartY = e.touches[0].clientY;
    const sl = currentSlideData();
    designer.dragStartOX = sl.imgOffsetX || 0;
    designer.dragStartOY = sl.imgOffsetY || 0;
  }, {passive:true});
  cardCanvas.addEventListener('touchmove', e => {
    if (!designer.isDragging || e.touches.length !== 1) return;
    const rect   = cardCanvas.getBoundingClientRect();
    const scaleX = SIZE / rect.width;
    const dx     = (e.touches[0].clientX - designer.dragStartX) * scaleX;
    const dy     = (e.touches[0].clientY - designer.dragStartY) * scaleX;
    const sl     = currentSlideData();
    sl.imgOffsetX = designer.dragStartOX + dx;
    sl.imgOffsetY = designer.dragStartOY + dy;
    renderCard();
  }, {passive:true});
  cardCanvas.addEventListener('touchend', () => { designer.isDragging = false; });

  // Scroll to zoom
  cardCanvas.addEventListener('wheel', e => {
    e.preventDefault();
    const sl    = currentSlideData();
    sl.imgScale = Math.max(0.5, Math.min(3, (sl.imgScale||1) - e.deltaY * 0.001));
    if (designer.type === 'single') designer.imgScale = sl.imgScale;
    renderCard();
  }, {passive:false});
}
...
function addCarouselSlide() {
  designer.slides.push({
    kicker:     designer.kicker,
    headline:   designer.headline,
    caption:    designer.caption,
    category:   designer.category,
    imageUrl:   designer.imageUrl,
    imageEl:    designer.imageEl,
    imgOffsetX: 0, imgOffsetY: 0, imgScale: 1,
  });
}
function currentSlideData() {
  if (designer.type === 'carousel' && designer.slides.length > 0) {
    return designer.slides[designer.currentSlide];
  }
  return designer;
}

document.getElementById('carousel-add-slide')?.addEventListener('click', () => {
  addCarouselSlide();
  designer.currentSlide = designer.slides.length - 1;
  syncSlideToUI(designer.currentSlide);
  updateCarouselUI();
  renderCard();
});
document.getElementById('carousel-del-slide')?.addEventListener('click', () => {
  if (designer.slides.length <= 1) return;
  designer.slides.splice(designer.currentSlide, 1);
  designer.currentSlide = Math.min(designer.currentSlide, designer.slides.length - 1);
  syncSlideToUI(designer.currentSlide);
  updateCarouselUI();
  renderCard();
});
document.getElementById('carousel-prev-slide')?.addEventListener('click', () => {
  if (designer.currentSlide > 0) {
    designer.currentSlide--;
    syncSlideToUI(designer.currentSlide);
    updateCarouselUI();
    renderCard();
  }
});
document.getElementById('carousel-next-slide')?.addEventListener('click', () => {
  if (designer.currentSlide < designer.slides.length - 1) {
    designer.currentSlide++;
    syncSlideToUI(designer.currentSlide);
    updateCarouselUI();
    renderCard();
...
function syncSlideToUI(idx) {
  const sl = designer.slides[idx];
  if (!sl) return;
  const fields = { 'cd-kicker':sl.kicker||'', 'cd-headline':sl.headline||'', 'cd-caption':sl.caption||'', 'cd-category':sl.category||'news', 'cd-imgurl':sl.imageUrl||'' };
  Object.entries(fields).forEach(([id,val]) => { const el=document.getElementById(id); if(el) el.value=val; });
}

function updateCarouselUI() {
  const wrap  = document.getElementById('carousel-controls');
  const ind   = document.getElementById('carousel-slide-indicator');
  const isC   = designer.type === 'carousel';
  if (wrap) wrap.style.display = isC ? 'flex' : 'none';
  if (ind) ind.textContent = isC ? 'Slide ' + (designer.currentSlide+1) + ' / ' + designer.slides.length : '';
}

/* ── DOWNLOAD ── */
document.getElementById('cd-download-btn')?.addEventListener('click', () => downloadCard('iol-card.png'));
document.getElementById('cd-download-all-btn')?.addEventListener('click', async () => {
  if (designer.type !== 'carousel' || !designer.slides.length) { downloadCard('iol-card.png'); return; }
  for (let i = 0; i < designer.slides.length; i++) {
    designer.currentSlide = i;
    syncSlideToUI(i);
    await renderCard();
    downloadCanvas('iol-slide-' + String(i+1).padStart(2,'0') + '.png');
...
document.getElementById('cd-generate-btn')?.addEventListener('click', async () => {
  ...
  try {
    const headline = document.getElementById('cd-headline')?.value || designer.headline;
    const caption  = document.getElementById('cd-caption')?.value  || designer.caption;
    const category = document.getElementById('cd-category')?.value || designer.category;
    const prompt = 'IOL social media card editor. Write card copy.\nStory: ' + headline + '\nDetails: ' + caption + '\nCategory: ' + category + '\nReturn ONLY raw JSON (no markdown, no fences): {"kicker":"2-4 WORD ALL-CAPS KICKER","headline":"punchy headline max 10 words","caption":"one compelling sentence max 18 words","shareText":"2-3 sentence social caption with hook"}';
...
async function renderCard() {
  await waitPoppins();
  const canvas = document.getElementById('card-canvas-draw');
  if (!canvas) return;
  canvas.width  = SIZE;
  canvas.height = SIZE;
  ...
  drawCard(ctx, {
    ...
    headline:   sl.headline  || designer.headline  || '',
    caption:    sl.caption   || designer.caption   || '',
    category:   (sl.category || designer.category  || 'news'),
    imageEl:    sl.imageEl   || designer.imageEl,
...
function drawCard(ctx, data) {
  ...
  /* 2. Photo — drag to reposition */
  if (data.imageEl) {
    ctx.save();
    ctx.beginPath(); ctx.rect(0, 0, S, S); ctx.clip();
    const sc     = Math.max(S / data.imageEl.width, S / data.imageEl.height) * (data.imgScale || 1);
    const iw     = data.imageEl.width  * sc;
    const ih     = data.imageEl.height * sc;
    const baseX  = (S - iw) / 2 + (data.imgOffsetX || 0);
    const baseY  = (S - ih) / 2 + (data.imgOffsetY || 0);
    ctx.drawImage(data.imageEl, baseX, baseY, iw, ih);
    ctx.restore();
  }

  /* 3. Dark gradient overlay — matches template */
  const g = ctx.createLinearGradient(0, 0, 0, S);
  g.addColorStop(0,    'rgba(0,0,0,0.18)');
  g.addColorStop(0.35, 'rgba(0,0,0,0.35)');
  g.addColorStop(0.62, 'rgba(0,0,0,0.52)');
  g.addColorStop(1,    'rgba(0,0,0,0.80)');
  ctx.fillStyle = g; ctx.fillRect(0, 0, S, S);

  /* 4. Category accent line at top-left (thin coloured strip) */
  ctx.fillStyle = c;
  ctx.fillRect(0, 0, 6, S);

  /* 5. Kicker — top-left white box exactly like template */
  ctx.save();
  ctx.font = '800 32px Poppins, Arial Black, sans-serif';
  ctx.textBaseline = 'middle';
  const kickText  = (data.kicker || '').toUpperCase();
  const kickW     = ctx.measureText(kickText).width + 40;
  const kickH     = 52;
  const kickX     = 48;
  const kickY     = 48;
  // White outline box
  ctx.strokeStyle = 'rgba(255,255,255,0.85)'; ctx.lineWidth = 2;
  ctx.strokeRect(kickX, kickY, kickW, kickH);
  ctx.fillStyle = 'rgba(0,0,0,0.2)'; ctx.fillRect(kickX, kickY, kickW, kickH);
  ctx.fillStyle = '#FFFFFF';
  ctx.fillText(kickText, kickX + 20, kickY + kickH / 2);
  ctx.restore();

  /* 6. Main headline — centred, bold */
  ctx.save();
  ctx.font         = '800 62px Poppins, Arial Black, sans-serif';
  ctx.fillStyle    = '#FFFFFF';
  ctx.textAlign    = 'center';
  ctx.textBaseline = 'alphabetic';
  const hlLines  = wrapText(ctx, data.headline || '', S - 96);
  const hlLH     = 74;
  const capLines = data.caption ? wrapTextFont(ctx, data.caption, '500 38px Poppins, Arial, sans-serif', S - 120) : [];
  const capLH    = 50;
  const gap      = 22;
  const logoH    = 100;
  const totalH   = hlLines.length * hlLH + (capLines.length ? gap + capLines.length * capLH : 0);
  ...
  if (capLines.length) {
    ctx.font      = '500 38px Poppins, Arial, sans-serif';
    ctx.fillStyle = 'rgba(255,255,255,0.90)';
    const capTop  = blockTop + hlLines.length * hlLH + gap;
    capLines.forEach((ln, i) => ctx.fillText(ln, S/2, capTop + (i+1) * capLH));
  }
  ctx.restore();
...
function wrapTextFont(ctx, text, font, maxW) {
  ctx.save(); ctx.font = font;
  const res = wrapText(ctx, text, maxW);
...
document.querySelector('.copy-btn[data-target="cd-share-text"]')?.addEventListener('click', function() {
  const el = document.getElementById('cd-share-text');
  if (!el || !el.value.trim()) return;
  navigator.clipboard.writeText(el.value).then(() => {
    const o = this.textContent; this.textContent = '✓ Copied!';
    setTimeout(() => this.textContent = o, 1800);
  });
});
...
document.addEventListener('DOMContentLoaded', () => {
  updateCarouselUI();
  renderCard();
  // Also re-render when cards panel becomes active
  document.querySelectorAll('.nav-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      if (tab.dataset.tab === 'cards') {
        populateCardStorySelector();
...
// Render on first load if cards panel is already active
if (document.getElementById('panel-cards')?.classList.contains('active')) {
  waitPoppins().then(() => renderCard());
}
